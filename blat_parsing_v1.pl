...
sub print {
	my $outputfilename;
	$outputfilename = "FBXoutput.txt";
	open (MYFILE, '>>'.$outputfilename);
    my ($self) = @_;
	if ($self->matchsize > 100){ # VERY IMPORTANT, WILL ONLY OUTPUT MATCHES >100 
	#print Blatline info
...
sub printAll
{
    ...
    $genomicend=$self->{_tstart}[$size] + $self->{_blkSz}[$size];
...
sub BlatlineMath{
	...
	##Use if statement here to filter out smaller than 100 size matches
